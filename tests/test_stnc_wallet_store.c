...
#define TEST_WALLET_FILE "wallet.key"
...
int main(void)
{
    ...
    if(stnc_wallet_store_create(&created)!=0||!stnc_wallet_store_exists()||
       stnc_wallet_address(&created,a)!=0||stnc_wallet_store_load(&loaded)!=0||
    ...
    if(stnc_wallet_store_load(&loaded)==0)return 1;
    {

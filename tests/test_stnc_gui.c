...
int main(void){char temp[MAX_PATH],dir[MAX_PATH]={0},path[MAX_PATH],exe[MAX_PATH],cmd[MAX_PATH+16];STARTUPINFOA si;PROCESS_INFORMATION pi={0};FILE *f;DWORD code;int failed=0,started=0;size_t i;const char *files[]={"stnc-core.exe","config.json","wallet.key","identity.key","stnc-core.log"};
 ...
 CHECK(GetDlgItem(app,300)!=NULL&&GetDlgItem(app,301)!=NULL&&GetDlgItem(app,302)!=NULL&&GetDlgItem(app,303)!=NULL&&GetDlgItem(app,304)!=NULL&&GetDlgItem(app,305)!=NULL);CHECK(GetDlgItem(app,102)!=NULL&&GetDlgItem(app,103)!=NULL);CHECK(GetDlgItem(app,107)!=NULL&&GetDlgItem(app,108)!=NULL);CHECK(GetDlgItem(app,112)!=NULL);
 /* Missing wallet: mining-on is unavailable. Create wallet and it becomes available. */
 CHECK(wait_control(107,FALSE)==0);click(102);CHECK(wait_control(107,TRUE)==0);
 /* Mining state is mutually exclusive. */
 click(107);CHECK(wait_control(107,FALSE)==0);CHECK(wait_control(108,TRUE)==0);click(108);CHECK(wait_control(107,TRUE)==0);CHECK(wait_control(108,FALSE)==0);
 /* Identity persists to its private file and enables Contract creation. */
 click(103);CHECK(wait_control(112,TRUE)==0);snprintf(path,sizeof(path),"%s\\wallet.key",dir);CHECK(GetFileAttributesA(path)!=INVALID_FILE_ATTRIBUTES);snprintf(path,sizeof(path),"%s\\identity.key",dir);CHECK(GetFileAttributesA(path)!=INVALID_FILE_ATTRIBUTES);CHECK(GetExitCodeProcess(pi.hProcess,&code)&&code==STILL_ACTIVE);
